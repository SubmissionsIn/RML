...
import argparse
import random
from RML_network import Network
from RML_loss import Loss
...
parser.add_argument('--dataset', type = str, default = 'nuswide10')
...
def add_noise(matrix, std, p):
    ...
    for i in range(rows):
        if random.random() < p:
            noise = torch.randn(cols, device=torch.device("cuda:0")) * std
...
def train(config, bit):
    ...
    our_criterion = Loss(256, 0.5, device).to(device)
    #############################
    ...
    our_parameters = list(net.parameters()) + list(txt_net.parameters()) + list(our_model.parameters())
    our_optimizer = torch.optim.SGD(our_parameters, lr=1e-4, momentum=0.9, weight_decay=1e-6)
    ...
    for epoch in range(config["epoch"]):
        ...
        for image, tag, tlabel, label, ind in train_loader:
            ...
            if clean_samples.sum() > 0:
                ...
                for view in range(2):
                    masked_x = mask_tensor[:, view].unsqueeze(1) * xs[view]
                    masked_xs.append(masked_x)
                for view in range(2):
                    noised_x = add_noise(xs[view], Gaussian_noise, noise_rate)
                    noised_xs.append(noised_x)

                _, mask_z, _, _, _ = our_model(masked_xs)
                _, noise_z, _, _, _ = our_model(noised_xs)
                ...
                # loss_con = Lambda * (loss_con_1 + loss_con_2)
...
if __name__ == "__main__":
    ...
    data_name_list = ['flickr', 'nuswide10']
