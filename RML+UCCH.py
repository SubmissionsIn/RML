# To run RML+UCCH on the following two datasets:
# cd RML+UCCH
# python RML+UCCH.py --data_name nus_wide_tc10_fea --bit ??? --max_epochs 20
# python RML+UCCH.py --data_name mirflickr25k_fea  --bit ??? --alpha 0.7 --num_hiden_layers 3 2 --margin 0.2 --max_epochs 20 --train_batch_size 256 --shift 0.1 --lr 0.0001 --optimizer Adam
# ??? = 16, 32, 64, 128
...
def main():
    ...
    def train(epoch):
        ...
        for batch_idx, (idx, images, texts, _) in enumerate(train_loader):
            ...
            # _, mask_z, _, _, _ = our_model(xs)
            # _, noise_z, _, _, _ = our_model(xs)
    ...
    i2t, t2i = test(chp['epoch'], is_eval=False)
